...
{
 public   class Program_FourthDay_Part4
    {
        ...
        public static void View<T>(T model, string replacePath)
        {
            var pathOutput = @"D:\abhay\kavita\kavitamvcclass\firstConsole\firstConsole\" + replacePath + ".html";

            var emp = new EmployeeTemplate(model);
            emp.Execute();
            System.IO.File.WriteAllText(pathOutput, emp.sb.ToString());
